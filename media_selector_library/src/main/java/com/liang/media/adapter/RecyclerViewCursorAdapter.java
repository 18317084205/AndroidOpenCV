...
public abstract class RecyclerViewCursorAdapter<VH extends RecyclerView.ViewHolder> extends
    ...
    public void onBindViewHolder(VH holder, int position) {
        if (!isDataValid(mCursor)) {
            throw new IllegalStateException("Cannot bind view holder when mCursor is in invalid state.");
        }
        if (!mCursor.moveToPosition(position)) {
            throw new IllegalStateException("Could not move mCursor to position " + position
    ...
    public long getItemId(int position) {
        if (!isDataValid(mCursor)) {
            throw new IllegalStateException("Cannot lookup item id when mCursor is in invalid state.");
        }
        if (!mCursor.moveToPosition(position)) {
            throw new IllegalStateException("Could not move mCursor to position " + position
        ...
        return mCursor.getLong(id_column);
    }
    ...
    public void swapCursor(Cursor newCursor) {
        ...
        if (newCursor != null) {
            mCursor = newCursor;
            id_column = mCursor.getColumnIndexOrThrow(MediaStore.Files.FileColumns._ID);
